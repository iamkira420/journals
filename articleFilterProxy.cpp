...
ArticleFilterProxy::ArticleFilterProxy(QObject *parent): QSortFilterProxyModel(parent), m_column(-1) , m_active(false)
{
...
bool ArticleFilterProxy::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    // QRegularExpression::wildcardToRegularExpression handles * and ?, and anchors the pattern to the full string by default.
    const QString regexStr = QRegularExpression::wildcardToRegularExpression(m_pattern);
    ...
    if (m_column >= 0) {
        ...
        // Pages == Col_Pages is excluded per the assignment specs. 
...
bool ArticleFilterProxy::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
    // For numeric columns, compare by the integer stored in Qt::UserRole to avoid lexicographic errors ("9" > "10" as strings).
    switch (left.column()) {
