 ...
 *
 * The current year is determined dynamically via QDate::currentDate() so the
 * application never needs to be updated for the calendar to roll over.
 */
...
class ArticleModel : public QStandardItemModel
{
     ...
     */
    bool addArticle(const QString &author,
                    int            year,
                    const QString &title,
     ...
     *
     * Overridden to inject Qt::BackgroundRole colouring based on the year
     * stored in Col_Year for the same row.  All other roles are delegated
     * to the base class.
     */
